...
4. **HTTP API** (`api/`) — `bunrouter` with `reqlog` middleware. Endpoints:
    - `GET /messages?page=&maxPerPage=` — paginated list (default 50/page, computes `pagesCount`). Messages are served as `types.MailDataResponse`, which embeds `*MailData` and shadows `Attachments` with a byte-less `AttachmentSummary` — `Attachment.Data`'s json tag exists for storm persistence, so serving `MailData` raw would base64 every attachment into every list response. New `MailData` fields are promoted automatically; only attachment-shaped changes need touching here.
