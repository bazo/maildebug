...
2. **Storage** (`storage/`) — `storm` ORM over `bbolt` (`go.etcd.io/bbolt`). The DB file lives at `data/<DB_NAME>` (default `data/mail.bolt`). `MailData` is the only registered model; `Date` is indexed. Raw RFC 822 message bytes are written separately to `data/messages/<id>` so they can be re-served if needed. Every listing sorts on `storage.orderField` = **`Id`** (capture order — `<UnixNano>-<hex>`, so lexicographic == chronological), never on `Date`: storm's `OrderBy` has no time.Time case and compares the marshalled RFC 3339 *text*, which ranks clock faces and ignores the zone offset, interleaving UTC-stamped mail (nodemailer always stamps UTC) with locally-stamped mail by exactly the offset. `session.Data` also stores `Date` as UTC so serialized timestamps have one shape. Pinned by `storage/ordering_test.go`.
3. **SMTP server** (`session/`) — built on `github.com/emersion/go-smtp`. `session.Backend` authenticates via `AuthPlain` against the configured username/password and parses incoming mail in `session.Data`:
    - `s.MaxLineLength` is set from `MAILDEBUG_MAX_LINE_LENGTH` (default 1 MiB, `0` = unlimited). go-smtp's own default is 2000 chars, and it answers a longer line with `500 5.4.0 Too long line, closing connection` (or `554 … too long a line in input stream` inside `DATA`/`BDAT`) and drops the connection — senders often surface that as a hang until their socket timeout. HTML renderers such as React Email emit the body unwrapped, so a single line of tens of thousands of chars is normal; the default must stay generous. Covered by the long-line tests in `email-test/send.test.tsx`.
